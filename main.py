...
# --- Recommendation Logic Parameters ---
TOP_N_FROM_MODEL = 20
# NEW: Define a pool of top candidates to shuffle for variety. Must be >= 3.
TOP_CANDIDATE_POOL_SIZE = 8 
...
def get_recommendations_with_info(user_input_dict, top_n_model, scaler, encoder, df_crop_global,
    ...
    # --- MODIFICATION: Introduce controlled randomness ---
    # 1. Select a small pool of the best candidates
    top_pool = filtered_candidates[:TOP_CANDIDATE_POOL_SIZE]

    # 2. Shuffle this small pool in-place to vary the top results
    random.shuffle(top_pool)

    # 3. Take the top 3 from the now-shuffled pool
    top_3_names = [item['crop'] for item in top_pool[:3]]

    if len(top_3_names) < 3: print(f"Warning: Found only {len(top_3_names)} suitable crops post-shuffle.")
    print(f"\nShuffling the top {len(top_pool)} candidates to provide variety.")
    print(f"Final Top {len(top_3_names)} Recommended Crop Names (post-shuffle): {top_3_names}")
